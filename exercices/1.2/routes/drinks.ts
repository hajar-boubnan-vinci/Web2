...
router.get("/", (_req, res) => {
   // Vérifiez si le paramètre "budget-max" est présent dans la requête
  if (!_req.query["budget-max"]) {
    // Si le paramètre n'est pas présent, renvoyez toutes les boissons
    return res.json(drinks);
  }

  // Convertissez le paramètre "budget-max" en nombre
  const budgetMax = Number(_req.query["budget-max"]);

  // Filtrez les boissons selon le budget maximum
  const filteredDrinks = drinks.filter((drink) => {
    return drink.price <= budgetMax;
  });

  // Renvoyez les boissons filtrées
